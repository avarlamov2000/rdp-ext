...
template <typename T>
concept PingType = requires(T ping, std::function<void()> handler) {
    { ping.start() } -> std::same_as<void>;
    { ping.stop() } -> std::same_as<void>;
    { ping.setPingHandler(handler) } -> std::same_as<void>;
};
...
template <TransportType Transport, ExecutorType Executor>
class Controller {
public:
    explicit Controller(Transport& transport, Executor& executor, logging::LogSink& log_sink)
        : transport_{transport}, executor_{executor}, state_machine_{log_sink}, serializer_{}, log_{log_sink, "client-controller"}
    {
        linkHandlers();
    }
...
private:
    ...
    void step(ClientEvent&& event) {
        const auto effects = state_machine_.doStep(event);
        handleEffects(effects);
    }

    void handleEffects(const ClientEffects& effects) {
    ...
    void handleEffect(const StartPingEffect& effect) {
    }

    void handleEffect(const StopPingEffect& effect) {
    }
